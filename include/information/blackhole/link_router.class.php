...
class Blackhole_Link_Router extends Link
{
	public $category = "Blackhole";
	public $type = "Blackhole_Link_Router";
	...
	public function html_list_row($i = 1)
	{
		...
		$OUTPUT .= <<<END
				...
				<tr class="{$rowclass}">
					...
					<td class="report" width="{$this->html_width[$i++]}">{$LINKED->data["ip"]}</td>
				</tr>
END;
		return $OUTPUT;
	}
...
END;
	...
	public function html_form()
	{
		...
		$OUTPUT .= $this->html_form_field_select("link"		,"Blackhole Router"		,$this->assoc_select_name($RESULTS)	);
	...
	// Need to write a quick and cute function to GET blackhole routes from the blackhole router...

	// Do we want an audit function here?
