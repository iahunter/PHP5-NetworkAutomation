...
class Security_Link_Service extends Link
{
	public $category = "Security";
	public $type = "Security_Link_Service";
	...
	public function html_list_header()
	{
		...
		<table class="report" width="{$this->html_width[0]}">
			<caption class="report">Service Link List</caption>
			<thead>
				<tr>
					<th class="report" width="{$this->html_width[$i++]}">ID</th>
					<th class="report" width="{$this->html_width[$i++]}">LinkID</th>
					<th class="report" width="{$this->html_width[$i++]}">Service</th>
...
END;
	...
	public function html_detail()
	{
		...
		<table class="report" width="{$this->html_width[0]}">
			<caption class="report">Service Link Details</caption>
			<thead>
				<tr>
					<th class="report" width="{$this->html_width[$i++]}">ID</th>
					<th class="report" width="{$this->html_width[$i++]}">LinkID</th>
...
END;
	...
	public function html_form()
	{
		...
		$SEARCH = array(
					"category"		=> "Security",
					"type"			=> "Service%",	// This is for future service child types
				);
		$RESULTS = Information::search($SEARCH,"stringfield1"); // Search for Services ordered by stringfield1 (name)
		$OUTPUT .= $this->html_form_field_select("link"		,"Service"	,$this->assoc_select_name($RESULTS)	);
