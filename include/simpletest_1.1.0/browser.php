...
class SimpleBrowser {
    ...
    public $page;
    public $history;
    public $ignore_frames;
    public $maximum_nested_frames;
    public $parser;
     ...
     *    @access public
     */
    function authenticate($username, $password, $realm = "") {
		if (! $realm) { $realm = $this->page->getRealm(); }
        if (! $realm) {
            return false;
        }
    ...
    /**
     *    Returns an array of ALL links objects on the page.
     *    @return array/SimpleSimpleAnchorTag
     *    @access public
     */
    function getLinks() {
        $urls = $this->page->getLinks();
        if (count($urls) == 0) {
            return false;
        }
        return $urls;
