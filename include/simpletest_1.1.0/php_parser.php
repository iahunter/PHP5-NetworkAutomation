...
class SimplePhpPageBuilder {
    public $tags;
    public $page;
    public $private_content_tag;
    public $open_forms = array();
    public $complete_forms = array();
    public $frameset = false;
    public $loading_frames = array();
    public $frameset_nesting_level = 0;
    public $left_over_labels = array();
    ...
    function free() {
        unset($this->tags);
        unset($this->page);
        unset($this->public_content_tags);
    ...
    function startElement($name, $attributes) {
        ...
        if ($tag->isPrivateContent() && ! isset($this->public_content_tag)) {
            $this->public_content_tag = &$tag;
        }
        if ($tag->expectEndTag()) {
            $this->openTag($tag);
            return true;
    ...
    function endElement($name) {
        ...
        if ($this->hasNamedTagOnOpenTagStack($name)) {
            ...
            if ($tag->isPrivateContent() && $this->public_content_tag->getTagName() == $name) {
                unset($this->public_content_tag);
            }
    ...
    function addContent($text) {
        if (isset($this->public_content_tag)) {
            $this->public_content_tag->addContent($text);
        } else {
